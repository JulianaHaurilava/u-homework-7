...
public class GameManager : MonoBehaviour
{
    ...
    private int WIN_USED_LEAFLETS_NUMBER = 75;                                             // минимальное количество распростарненных листовок для победы
        ...
        if (FightTimer.TaskCompleted)
        {
            ...
            if (fightsAlive == MAX_FIGHT_NUMBER)
            {
                if (usedLeaflets < WIN_USED_LEAFLETS_NUMBER || WarriorsAmount < 1)
                {
                    ...
                    // после финального столкновения не остается ни одного бойца
                    LooseGame();
                }
                else
                {
    ...
    /// <summary>
    /// Рассчитывает и обновляет индекс скорости генерации воинов
    /// </summary>
